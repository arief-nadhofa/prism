...
<main class="flex-1 p-4 lg:p-8 space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
                <h3 class="font-bold text-slate-900 text-base">List Category</h3>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/60 transition">
                        ...
                        <td class="px-6 py-4 font-medium text-slate-800">{{$c->category}}</td>
                        <td class="px-6 py-4 text-right">

                            <button type="button"
                                onclick="toggleModal('modal-edit-{{$c->id}}')"
                            ...
                            <form action="{{ route('category.destroy', $c->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?');" class="inline">
                                @csrf
                                @method('DELETE')
                    ...
                    <!-- Modal Container -->
                    <div id="modal-edit-{{$c->id}}" class="fixed inset-0 z-50 hidden items-center justify-center p-4">

                        <div onclick="toggleModal('modal-edit-{{$c->id}}')" class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs transition-opacity"></div>
                        <div class="relative bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden transform transition-all">
                            ...
                            <!-- Body / Form -->
                            <form method="POST" action="{{ route('category.update',$c->id) }}" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-y-auto">
                                @csrf
                                @method('PUT')
                                <div class="p-6 space-y-4">
                                    <div>
                                        <label for="category" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Category Name</label>
                                        <input name="category" id="category" value="{{ old('category', $c->category) }}"
            ...
            <div id="modal-tambah" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden transform transition-all">
                    ...
                    <!-- Body / Form -->
                    <form method="POST" action="{{ route('category.store') }}" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-y-auto">
                        @csrf
                        <div class="p-6 space-y-4">
                            <div>
                                <label for="category" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Category Name</label>
                                <input name="category" id="category" placeholder="Contoh: ERP/Internal System"
                                    class="w-full px-3.5 py-2 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-4 focus:ring-indigo-600/10 focus:border-indigo-600 transition" required />
                            </div>
